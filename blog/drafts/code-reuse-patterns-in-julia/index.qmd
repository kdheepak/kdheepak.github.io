...
struct Light
  color::String
  Light(color::Union{Nothing, String}=nothing) = new(isnothing(color) ? "Red" : color)
end
...
struct BlinkingLight
  light::Light
  blink_speed::String
  BlinkingLight(color::Union{Nothing, String}=nothing, blink_speed::Union{Nothing, String}=nothing) = new(Light(color), isnothing(blink_speed) ? "Fast" : blink_speed)
end
...
struct SoundAlert
  light::Light
  sound_type::String
  SoundAlert(color::Union{Nothin, String}=nothing, sound_type::Union{Nothing, String}=nothing) = new(Light(color), isnothing(sound_type) ? "Ping" : sound_type)
end
...
struct PedestrianCrossingLight
  ...
  PedestrianCrossingLight(color::Union{Nothing, String}=nothing, blink_speed::Union{Nothing, String}=nothing, sound_type::Union{Nothing, String}=nothing) = new(BlinkingLight(color, blink_speed), SoundAlert(color, sound_type))
end
...
function Base.getproperty(pcl::BlinkingLight, sym::Symbol)
    if sym === :color
...
struct SoundAlert
    light::Light
    sound_type::String
    SoundAlert(color::Union{Nothing, String}=nothing, sound_type::Union{Nothing, String}=nothing) = new(Light(color), isnothing(sound_type) ? "Ping" : sound_type)
end
...
function Base.getproperty(pcl::PedestrianCrossingLight, sym::Symbol)
    if sym === :color
