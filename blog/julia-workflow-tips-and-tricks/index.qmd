...
::: callout-note
Keep in mind that these packages will be added to your root environment. This means they are not available to import inside a package, and only in a REPL environment.
Also, if you want to ensure you have reproducible environments for yourself and for the people you share your code with, you should use dedicated per-project local environments for that, instead of adding packages to your global environment.

I personally only add packages that improve my developer workflow, e.g. `Revise`, `BenchmarkTools`, `OhMyREPL`, to my root environment.
:::
...
[`PkgTemplates.jl`](https://github.com/invenia/PkgTemplates.jl) is a utility package that makes the process of creating new packages efficient, systematic, and customizable.
`PkgTemplates` offers a simplified way to generate new Julia packages with numerous features out-of-the-box that adhere to best practices (e.g. GitHub Actions, Documenter, etc).
...
`JET` serves as a "linter" – not only flagging syntax errors but also identifying potential runtime errors without actually executing the code.
...
You can see the other kinds of warnings that JET can produce in the [`demo.jl`](https://github.com/aviatesk/JET.jl/blob/master/demo.jl).
